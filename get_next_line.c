...
/*   Updated: 2020/12/28 14:38:28 by otaishi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int				get_next_line(int fd, char **line)
{
	...
	if (!line || read(fd, save, 0) < 0 || BUFFER_SIZE <= 0 ||
			!(buf = (char *)malloc(sizeof(char) * ((size_t)BUFFER_SIZE + 1))))
		return (ERROR);
	*line = NULL;
	if (!save)
		return (ret = read_process(fd, buf, &save, line));
	if ((ret = save_in_line(&save, line)) == CONTINUE_READ)
		return (ret = read_process(fd, buf, &save, line));
	free_ptr(&buf);
	return (ret);
}
